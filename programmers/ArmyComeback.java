...
public class ArmyComeback {
    public int[] solution(int n, int[][] roads, int[] sources, int destination) {
        int[] answer = new int[sources.length];
        
        List<Integer>[] list = getList(n, roads);
        int[] path = findReverse(n, destination, list);
        //System.out.println();
    ...
    public int[] findReverse(int n, int dest, List<Integer>[] list) {
    	int[] paths = new int[n];
    	Arrays.fill(paths, -1);
    	...
    	while(s.size() > 0) {
    		...
    		for (int i = 0; i < list[cur].size(); i++) {
    			...
    			if (paths[next] == -1 || paths[next] > paths[cur] + 1) {
    				s.push(next);
    				paths[next] = paths[cur] + 1;
    			} else if ((paths[cur]) > paths[next] + 1) {
    				paths[cur] = paths[next] + 1;
    			}
    		}
    	}
	...
	public List<Integer>[] getList(int n, int[][] roads) {
